...
| Frontend | **React 18 + TypeScript + Vite**; **TanStack Query** (server state — pagination, counts, optimistic read/star, invalidation), **TanStack Router** (type-safe routing — pairs with the generated API types), **Radix UI** primitives + **CSS Modules** for styling, **lucide-react** icons, **`vite-plugin-pwa`** + **`idb`** (offline, WP-14). Types are **generated** from the API's OpenAPI schema via `openapi-typescript` — no hand-maintained interfaces. **No full component kit** (MUI/Chakra/Mantine/Ant) and **no date library**: a headless-primitives + own-CSS approach fits the dense §1.7 aesthetic and the bundle budget, and dates use the built-in `Intl.RelativeTimeFormat`/`Intl.DateTimeFormat`. | Bundle budget ≤ 180 KB gz (React + Clerk cost more than Svelte; budget acknowledges reality but still gates growth). Rule of thumb: reach for a library for genuinely hard, error-prone wheels (server-state cache, a11y overlays, service worker, IndexedDB); hand-roll the trivial and the product-defining (rate limiting, SSRF guard, keyboard nav). |
...
**Component strategy — headless primitives, own CSS, no kit.** The look is deliberately custom and dense, so a batteries-included component kit (MUI/Chakra/Mantine/Ant) is the wrong tool: it fights the aesthetic, bloats the bundle, and makes everything "look like the kit." Instead, ~90% of the UI is plain styled elements (rows, panes, sidebar) in **CSS Modules** with a small design-token layer (CSS custom properties). The handful of genuinely interactive overlays — the "move to folder" menu, the add-subscription and OPML-import dialogs, timestamp tooltips, the density toggle — use **Radix UI** *unstyled* primitives, which supply the hard, error-prone parts (focus management, ARIA, keyboard, dismiss) without imposing any style. Icons are **lucide-react** (never hardcoded emoji). This is the mainstream approach for custom-looking, accessibility-serious products and directly serves the WP-12 keyboard/a11y goals; Tailwind + shadcn/ui is the documented alternative if a utility-CSS workflow is ever preferred.
...
6. **Scale & cost:** the p95 < 100 ms budget is benchmark-gated in WP-13 (100k entries per-PR, 5M nightly). A plain GIN turned out **insufficient**: it finds matches unordered, so the chronological `ORDER BY id DESC LIMIT 50` sorts the whole match set and the tail blows the budget (~166 ms p95 at 5M). The index is therefore the **`rum` extension** with the id attached (`rum (search_tsv rum_tsvector_addon_ops, id)`), and the query orders by `id <=| :anchor` — matches come back already id-ordered from the index, no sort (~35 ms p95 at 5M, 5 cold runs). `rum` isn't in core Postgres, so the DB is a small custom image (`deploy/Dockerfile.postgres`). Index write amplification at poller insert rates is still negligible. (Migration 0003; the rare feed-name arm falls back to a sort and is resolved out of the hot path.)
